...
def year_cols(ws, header_row=4):
    ...
    for ci in range(3, 20):
        ...
        if isinstance(v, str) and re.fullmatch(r"FY\d{2}[AE]?", v):
            out[v[:4]] = get_column_letter(ci)   # キーはA/Eサフィックスを除いたFYxx
    return out
...
def detect_b_only(xlsx_path, pdf_texts):
    ...
    if "calc" in wb.sheetnames:
        below = [c.coordinate for row in wb["calc"].iter_rows(min_row=23) for c in row
                 if c.value is not None]
        if below:
            add("E10", f"calc下部:{below[0]}〜（{len(below)}セル）")
    full = "\n".join(pdf_texts)
    if "百万円" in full:
        add("X2", "DDに百万円表記")
    if "5,280" in full:
...
def main():
    ...
    n_periods = [len(year_cols(wa[s])) for s in ("PL", "BS", "CF", "calc", "inp")]
    ...
    def unit_patterns(wb, has_debt):
        ...
        if wb["BS"]["A2"].value == "（単位：千円）":
            pats.append("BS!A2")
        if has_debt and wb["debt"]["A2"].value == "(Unit: JPY thousands)":
    ...
    def english_ratio(wb):
        labels = [v for c, v in sheet_strings(wb["debt"]) if c.startswith("B")]
    ...
    n_periods_b = [len(year_cols(wb1[s])) for s in ("PL", "BS", "CF", "calc", "inp")]
    ...
    def monthly_tie_max(wb, ev):
        ...
        for key in rows:
            ...
            # 年次PLの費用行は負値規約のため大きさ（絶対値）で突合する
            max_dev = max(max_dev, abs(abs(msum) / 1000 - abs(annual)))
        return max_dev
    ...
    def side_blocks(wb):
        ...
        ws = wb["calc"]
        cells = [(c.row, c.column) for row in ws.iter_rows(min_row=23) for c in row
                 if c.value is not None and c.row >= 23]
        if cells:
            blocks.append(("calc下部", len({r for r, _ in cells}), len({cc for _, cc in cells})))
        return blocks
    ...
    x3_b = int("5,280" in b_pdf_full) + int("1,585" in b_text_all or "1,585" in str(
        wb1["inp"]["D28"].value))
    teaser = wb1["inp"]["D28"].value
    x3_b = int("5,280" in b_pdf_full and wb1["inp"]["D21"].value == 5_500_000) \
