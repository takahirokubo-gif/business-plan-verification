...
def _facts() -> list[dict]:
    ...
    for y, col in (("FY32", "M"), ("FY33", "N")):
        ...
        add(f"bill_{y.lower()}_base", f"計画（{y}・新規作成）", "派遣単価（Base）",
            _drv[y]["bill"], "円/h", f"{A_XLSX}|calc!{col}11", f"{B_XLSX}|calc!{col}11")
        add(f"util_{y.lower()}_base", f"計画（{y}・新規作成）", "稼働率（Base）",
            _drv[y]["util"], "－", f"{A_XLSX}|calc!{col}8", f"{B_XLSX}|calc!{col}8")

    # ---- デットスケジュール
    add("interest_fy27", "デット", "支払利息（FY27）", _sched["FY27"]["interest"], "千円",
        f"{A_XLSX}|debt!H19", f"{DEBT_B}|debt!H19",
    ...
    add("sign_convention", "表記規約", "費用・返済・支払利息の符号（モデル上）",
        "負値表記（縦計算はSUM）。factsの正解値は正の金額で保持", "－",
        f"{A_XLSX}|PL!B34注記", f"{B_XLSX}|PL!B34注記")
    return F
...
def _planted() -> list[list]:
    items = []
    for f in ("planted_items_models.json",):   # リアリティ版はDD分もmodelsに統合済み
