...
- 日本語TTFフォントを埋め込む（pypdfでのテキスト抽出＝validate.pyの検証が可能になる）
...
class Page:
    ...
    def __init__(self, c: canvas.Canvas, doc_title: str, page_no: int, label: str):
        ...
        c.drawRightString(PAGE_W - MARGIN_R, PAGE_H - 38, "Project Gear")
            ...
            for j, lines in enumerate(cell_lines):
                ...
                for ln in lines:
                    ...
                    if i > 0 and j > 0 and ln and all(
                            ch in "0123456789,.%△+－-()倍年hx" for ch in ln):
...
DISCLAIMER = [
    ...
    "株式会社オートスタッフ中部に関するデューデリジェンスの結果を取りまとめたものであり、"
...
def render_doc(doc_key: str) -> tuple[Path, int]:
    ...
    c.setFont(FONT, 10)
    c.setFillColorRGB(*GRAY)
    c.drawCentredString(PAGE_W / 2, 470, "（Project Gear）")
    c.setFillColorRGB(*BODY_COLOR)
    c.setFont(FONT, 11)
