...
YEAR_SHEETS = [L.SHEET_PL, L.SHEET_BS, L.SHEET_CF, L.SHEET_KPI, L.SHEET_ASSUMPTIONS]
...
def check_common_excel(tag, path, variant, case):
    ...
    check("E1/E5", ds, f"{tag}: 全対象シートの期間列がFY22〜FY33の12期・実績/計画区分一致",
    ...
    for sname in YEAR_SHEETS + ([L.SHEET_DEBT] if variant == "A" else []):
        for y in YEARS:
            ...
            if not (isinstance(v, str) and re.fullmatch(r"FY\d{2}", v)):
    ...
    if case == "base":
        ...
        w_util = [wb["KPI_Drivers"][f"{COL[y]}{L.KPI_ROWS['util'][0]}"].value for y in ("FY32", "FY33")]
...
def check_integrity(tag, path, variant, case):
    ...
    def expect(sheet, row_key, y, want, rows=None):
        rows = rows or {"PL": L.PL_ROWS, "BS": L.BS_ROWS, "CF": L.CF_ROWS,
                        "KPI_Drivers": L.KPI_ROWS, "Debt_Schedule": L.DEBT_ROWS}[sheet]
    ...
    # PL全行（Bの計画期支払利息は破損セルなのでスキップし、正値との照合は ordinary で担保）
    for y in YEARS:
    ...
    for y in YEARS:
        for key in ("enrolled", "util", "active", "hours", "bill", "wage",
            ...
            expect("KPI_Drivers", key, y, drv[y][key])
    # Debt（Aは本体、Bは別ファイル）
    ...
    for y in PLAN_YEARS:
        for key in ("opening", "repayment", "closing", "interest"):
            ...
            got = dev.value("Debt_Schedule", coord)
            want = sched[y][key]
            if got is None or abs(float(got) - want) > 0.5:
    ...
    if variant == "B":
        ...
        check("E11", "B", f"{tag}: 支払利息FY27〜33の7セルが解決不能参照（#REF!）・全番地planted記載",
    ...
    for ws in wb.worksheets:
        for row in ws.iter_rows():
            for c in row:
                ...
                if ws.title in ("PL", "BS", "KPI_Drivers") and c.value is not None \
                        and c.column >= 16:
...
def check_b_specific():
    for tag, path in (("Base_B", B_BASE), ("Sponsor_B", B_SPONSOR)):
        ...
        side = []
        pl_ws, kpi_ws, bs_ws = wb["PL"], wb["KPI_Drivers"], wb["BS"]
        ...
        total = sum(kpi_ws[f"C{21 + i}"].value for i in range(3))
        kpi_ok = (kpi_ws["B19"].value or "").startswith("■") and total == drv["FY26"]["hires"] \
            and kpi_ws[f"C{24}"].value == drv["FY26"]["hires"]
        side.append(("KPI B19:D24", kpi_ok, 5, 3))
    ...
    labels_same = all(
        ...
        for s in ("PL", "BS", "CF", "KPI_Drivers") for r in range(4, 35))
    ...
    ok = B_DEBT.exists() and "Debt_Schedule" in load(B_DEBT).sheetnames
